...
/**
 * Suspense-enabled hook that fetches the doctors list using the TRPC client and the current
 ...
 */
export const useSuspenseDoctors = () => {
    const trpc = useTRPC();
    const [params] = useDoctorsParams();
    return useSuspenseQuery(trpc.doctors.getMany.queryOptions(params));
}
